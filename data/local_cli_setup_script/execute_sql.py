...
Generates synthetic education data for EduPath Academy.
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate synthetic education data (EduPath Academy) in Databricks Unity Catalog."
    )
    ...
    print("\n=== 1. Creating customers table (200 students) ===")
    exec_sql(f"""
    ...
    count = batch_insert(f"{FULL_SCHEMA}.customers",
        ...
        rows, token, host, wid)
    print(f"  Total: {count} students inserted")

    # ── 2. Courses (products table) ─────────────────────────────────────
    print("\n=== 2. Creating products table (~500 courses) ===")
    exec_sql(f"""
    ...
    for cat in PRODUCTS_BY_CATEGORY:
        ...
        for name, instructor, price, unit in PRODUCTS_BY_CATEGORY[cat]:
            products.append({"product_id": f"PROD-{pid:04d}", "name": name, "category": cat, "brand": instructor, "price": price, "stock_quantity": random.randint(15, 120), "aisle": buildings[cat], "unit": unit})
            pid += 1
    ...
    while len(products) < 500:
        ...
        products.append({"product_id": f"PROD-{pid:04d}", "name": f"{variation}{base[0]}", "category": cat, "brand": base[1], "price": round(base[2] * random.uniform(0.8, 1.5), 2), "stock_quantity": random.randint(15, 120), "aisle": buildings[cat], "unit": base[3]})
        pid += 1
    ...
    print(f"  Total: {count} courses inserted")

    # ── 3. Campuses (stores table) ───────────────────────────────────────
    print("\n=== 3. Creating stores table (10 campuses) ===")
    exec_sql(f"""
    ...
    rows = []
    store_ids = []
    for i, sname in enumerate(CAMPUS_NAMES, 1):
    ...
    count = batch_insert(f"{FULL_SCHEMA}.stores",
        ...
        rows, token, host, wid)
    print(f"  Total: {count} campuses inserted")

    # ── 4. Enrollments (transactions) + Course Items ─────────────────────
    print("\n=== 4. Creating transactions table (2000 enrollments) ===")
    exec_sql(f"""
    ...
    for txn_id in range(1, 2001):
        ...
        for prod in txn_products:
            ...
            total += line_total
            item_rows.append(f"('ITEM-{item_id:06d}', 'TXN-{txn_id:05d}', '{prod['product_id']}', {qty}, {unit_price}, {discount})")
            item_id += 1
    ...
    print(f"  Total: {count} enrollments inserted")
    ...
    print(f"  Total: {count} course enrollments inserted")

    # ── 5. Tuition Payment History ──────────────────────────────────────
    print("\n=== 5. Creating payment_history table (400 tuition payments) ===")
    exec_sql(f"""
    ...
    count = batch_insert(f"{FULL_SCHEMA}.payment_history",
        ...
        rows, token, host, wid)
    print(f"  Total: {count} tuition payment records inserted")
